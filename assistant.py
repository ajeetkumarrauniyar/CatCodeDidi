...
Progress is reported through the `emit(kind, payload)` callback supplied by
whatever front end is driving it - a terminal, or a window - so the pipeline
itself stays free of any presentation concern. All methods block, and a
threaded caller is expected to run them off its main thread.
"""
